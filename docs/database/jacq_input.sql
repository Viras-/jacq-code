...
-- Table `rev_flora`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `rev_flora` ;

CREATE TABLE IF NOT EXISTS `rev_flora` (
  `rev_flora_id` INT NOT NULL AUTO_INCREMENT,
  `uuid_minter_id` INT NOT NULL,
  PRIMARY KEY (`rev_flora_id`),
  INDEX `fk_rev_flora_srvc_uuid_minter_idx` (`uuid_minter_id` ASC),
  CONSTRAINT `fk_rev_flora_srvc_uuid_minter`
    FOREIGN KEY (`uuid_minter_id`)
    REFERENCES `srvc_uuid_minter` (`uuid_minter_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
...
-- Table `frmwrk_group`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `frmwrk_group` ;

CREATE TABLE IF NOT EXISTS `frmwrk_group` (
...
-- -----------------------------------------------------
-- Table `frmwrk_user_group`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `frmwrk_user_group` ;

CREATE TABLE IF NOT EXISTS `frmwrk_user_group` (
  `user_id` INT NOT NULL,
  `group_id` INT NOT NULL,
  PRIMARY KEY (`user_id`, `group_id`),
  INDEX `fk_frmwrk_user_group_frmwrk_user1_idx` (`user_id` ASC),
  INDEX `fk_frmwrk_user_group_frmwrk_group1_idx` (`group_id` ASC),
  CONSTRAINT `fk_frmwrk_user_group_frmwrk_user1`
    FOREIGN KEY (`user_id`)
    REFERENCES `frmwrk_user` (`id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION,
  CONSTRAINT `fk_frmwrk_user_group_frmwrk_group1`
    FOREIGN KEY (`group_id`)
    REFERENCES `frmwrk_group` (`group_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
-- -----------------------------------------------------
-- Placeholder table for view `view_botanical_object_vegetative`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `view_botanical_object_vegetative` (`id` INT, `derivative_id` INT, `scientific_name` INT, `accession_number` INT, `label_annotation` INT, `type` INT);

-- -----------------------------------------------------
-- Placeholder table for view `view_botanical_object_living`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `view_botanical_object_living` (`id` INT, `derivative_id` INT, `scientific_name` INT, `accession_number` INT, `label_annotation` INT, `type` INT);
...
USE `jacq_input`;
CREATE  OR REPLACE VIEW `view_derivative_vegetative_count` AS select `vd`.`living_plant_id` AS `living_plant_id`,count(0) AS `vegetative_count` from `tbl_derivative_vegetative` `vd` group by `vd`.`living_plant_id`;
...
CREATE  OR REPLACE VIEW
            `view_botanical_object_search` AS
select
          `bo`.`id`                             AS `id`
        ...
        ,`lp`.`label_annotation`                AS `label_annotation`
...
select
        ...
        ,`vd`.`separated`                                                                              AS `separated`
        ,`vd`.`index_seminum`                                                                          AS `index_seminum`
        ,`o`.`greenhouse`                                                                              AS `greenhouse`
        , CONCAT_WS('-', LPAD(`lp`.`accession_number`, 7, '0'), LPAD(`vd`.`accession_number`, 3, '0')) AS `accession_number`
        ,`lp`.`label_annotation`                                                                       AS `label_annotation`
...
-- -----------------------------------------------------
-- View `view_botanical_object_vegetative`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `view_botanical_object_vegetative` ;
DROP TABLE IF EXISTS `view_botanical_object_vegetative`;
USE `jacq_input`;
CREATE  OR REPLACE VIEW
            `view_botanical_object_vegetative` AS

select
          `bo`.`id`                                                                                     AS `id`
        ,`vd`.`derivative_vegetative_id`                                                                AS `derivative_id`
        , GetScientificName(`bo`.`scientific_name_id`, 0, 0)                                            AS `scientific_name`
        , CONCAT_WS('-', LPAD(`lp1`.`accession_number`, 7, '0'), LPAD(`vd`.`accession_number`, 3, '0')) AS `accession_number`
        ,`lp1`.`label_annotation`                                                                       AS `label_annotation`
        ,'vegetative'                                                                                   AS `type`
from
          `tbl_derivative_vegetative` `vd`
          left join
                    `tbl_living_plant` `lp1`
                    on
                              `lp1`.`id` = `vd`.`living_plant_id`
          left join
                    `tbl_botanical_object` `bo`
                    on
                              `bo`.`id` = `lp1`.`id`;
...
-- -----------------------------------------------------
-- View `view_botanical_object_living`
-- -----------------------------------------------------
DROP VIEW IF EXISTS `view_botanical_object_living` ;
DROP TABLE IF EXISTS `view_botanical_object_living`;
USE `jacq_input`;
CREATE  OR REPLACE VIEW `view_botanical_object_living` AS
...
USE `jacq_input`$$
DROP TRIGGER IF EXISTS `tbl_derivative_vegetative_BEFORE_INSERT` $$
USE `jacq_input`$$
CREATE DEFINER = CURRENT_USER TRIGGER `jacq_input`.`tbl_derivative_vegetative_BEFORE_INSERT` BEFORE INSERT ON `tbl_derivative_vegetative` FOR EACH ROW
BEGIN
	DECLARE v_accession_number INT;
    
    SET v_accession_number = (SELECT max(accession_number) FROM tbl_derivative_vegetative WHERE living_plant_id = NEW.living_plant_id);
    
    SET NEW.accession_number = IFNULL(v_accession_number, 0) + 1;
