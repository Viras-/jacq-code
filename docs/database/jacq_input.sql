...
CREATE TABLE IF NOT EXISTS `tbl_derivative_vegetative` (
  ...
  UNIQUE INDEX `accesion_number_UNIQUE` (`accession_number` ASC),
...
USE `jacq_input`;
CREATE  OR REPLACE ALGORITHM=UNDEFINED DEFINER=`jacq`@`%` SQL SECURITY DEFINER VIEW `view_botanical_object_search` AS select `bo`.`id` AS `id`,`lp`.`id` AS `derivative_id`,`bo`.`scientific_name_id` AS `scientific_name_id`,`vt`.`genus` AS `genus`,`vt`.`epithet` AS `epithet`,`bo`.`organisation_id` AS `organisation_id`,`o`.`description` AS `organisation_description`,`l`.`location` AS `location`,`lp`.`place_number` AS `place_number`,`bo`.`separated` AS `separated`,`lp`.`index_seminum` AS `index_seminum`,`o`.`greenhouse` AS `greenhouse`,`lp`.`accession_number` AS `accession_number`,ifnull(`dvc`.`vegetative_count`,0) AS `vegetative_count`,'living' AS `type` from ((((((`tbl_living_plant` `lp` left join `tbl_botanical_object` `bo` on((`bo`.`id` = `lp`.`id`))) left join `view_taxon` `vt` on((`vt`.`taxonID` = `bo`.`scientific_name_id`))) left join `tbl_organisation` `o` on((`o`.`id` = `bo`.`organisation_id`))) left join `tbl_acquisition_event` `ae` on((`ae`.`id` = `bo`.`acquisition_event_id`))) left join `tbl_location` `l` on((`l`.`id` = `ae`.`location_id`))) left join `view_derivative_vegetative_count` `dvc` on((`dvc`.`living_plant_id` = `lp`.`id`))) union select `bo`.`id` AS `id`,`vd`.`derivative_vegetative_id` AS `derivative_id`,`bo`.`scientific_name_id` AS `scientific_name_id`,`vt`.`genus` AS `genus`,`vt`.`epithet` AS `epithet`,`vd`.`organisation_id` AS `organisation_id`,`o`.`description` AS `organisation_description`,`l`.`location` AS `location`,`lp`.`place_number` AS `place_number`,`bo`.`separated` AS `separated`,`vd`.`index_seminum` AS `index_seminum`,`o`.`greenhouse` AS `greenhouse`,`vd`.`accession_number` AS `accession_number`,NULL AS `vegetative_count`,'vegetative' AS `type` from ((((((`tbl_derivative_vegetative` `vd` left join `tbl_living_plant` `lp` on((`lp`.`id` = `vd`.`living_plant_id`))) left join `tbl_botanical_object` `bo` on((`bo`.`id` = `lp`.`id`))) left join `view_taxon` `vt` on((`vt`.`taxonID` = `bo`.`scientific_name_id`))) left join `tbl_organisation` `o` on((`o`.`id` = `vd`.`organisation_id`))) left join `tbl_acquisition_event` `ae` on((`ae`.`id` = `bo`.`acquisition_event_id`))) left join `tbl_location` `l` on((`l`.`id` = `ae`.`location_id`)));
