    ...
    public function classificationAccess($taxonID, $user_id) {
        ...
        foreach ($models_taxSynonymy as $model_taxSynonymy) {
            ...
            while ($currentClassificationAccess === NULL) {
                ...
                // find parent entry
                $tax_syn_ID = $model_taxSynonymy->tax_syn_ID;
                $model_taxSynonymy = $model_taxSynonymy->getParent();

                // break an endless loop due to wrong data
                if ($model_taxSynonymy != null && $tax_syn_ID == $model_taxSynonymy->tax_syn_ID) {
                    Yii::log('Self Reference in Classification - check data! [' + $tax_syn_ID + ']', CLogger::LEVEL_ERROR);
                    break;
                }

                // no more parents are left, abort access check
