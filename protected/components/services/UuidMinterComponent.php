...
class UuidMinterComponent extends CApplicationComponent {
    ...
    /**
     * Mint an uuid for a given specimen
     * @param int $specimen_id ID of specimen in internal system
     * @return string UUID for specimen id
     * @throws Exception
     */
    public function specimen($specimen_id) {
        return $this->mint(self::SPECIMEN_TYPE_ID, $specimen_id)->uuid;
    }

    /**
     * Return the uuid for a given specimen, prefixed for URL referencing
     * @param int $specimen_id ID of specimen in internal system
     * @return string URL including the UUID for resolving
     * @throws Exception
     */
    public function specimenUrl($specimen_id) {
        return Yii::app()->params['guidUrlPrefix'] . $this->specimen($specimen_id);
    }
